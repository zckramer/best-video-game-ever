...
2. **Do NOT suggest Blueprint-only solutions** - Even if it seems faster/easier
...
- "Add a sprint ability to the player"
  → Implement `SetSprintActive()`, `GetSprintSpeed()` in C++
- "Create an inventory system"
  → Build `UInventoryComponent` in C++, let Blueprint handle UI
- "Add enemy AI"
  → Implement `AAIEnemy` base class in C++, extend in Blueprint for variants

**Red Flags (Don't Do These):**
- ❌ "Let's just make this in Blueprint" (without C++ base first)
- ❌ "We'll implement the logic in Blueprint later" (we won't)
- ❌ "This is too complex for C++, use Blueprint" (it's not, and use C++)
